...
| NFA-BETR-13 | Mehrere Instanzen gegen eine Datenbank | MUSS | T | M17 | umgesetzt | PostgreSQL statt SQLite; `connection_limit=1` entfallen. Kein Zustand im Prozess — alle zehn Fundstellen mit Modulzustand sind Zwischenspeicher oder Singletons. Die Lückenlosigkeit hängt seither daran, dass `incrementSequence` ein einziges `INSERT … ON CONFLICT DO UPDATE` ist |
...
| A23 | Zwei Instanzen | offen | Von Hand: gemeinsame Sitzung, nebenläufiges Festschreiben ohne Lücke, PDF über den Objektspeicher, Renderdienst anhalten, `RUN_MIGRATIONS=0` gegen veralteten Stand. Automatisiert belegt sind die Teile in `file-store.test.ts` und `renderer-service.test.ts` |
