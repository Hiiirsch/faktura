...
| FA-ADM-08 | Zweiter Faktor für Betreiberkonten verpflichtend | MUSS | T | M8 | umgesetzt | `adminLogin` endet immer mit einem Nachweis, nie mit einer Sitzung. Und es gibt zu keinem Zeitpunkt ein Konto ohne zweiten Faktor: Es entsteht erst beim Einlösen des Einrichtungslinks, mit `totpEnabled: true` in derselben Transaktion. `tests/integration/admin-session.test.ts` |
